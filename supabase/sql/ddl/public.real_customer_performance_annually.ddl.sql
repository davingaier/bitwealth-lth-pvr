create view public.real_customer_performance_annually as
with
  d as (
    select
      dd.customer_id,
    ...
    from
      real_customer_performance_daily dd
  ...
  agg as (
    select
      filt.customer_id,
      filt.trading_year,
      max(filt.tx_date) as latest_tx_date,
      sum(filt.omega_buy_usdt) as total_omega_buy_usdt,
      sum(filt.omega_sell_usdt) as total_omega_sell_usdt,
      sum(filt.sab_buy_usdt) as total_sab_buy_usdt,
      sum(filt.omega_buy_btc) as total_omega_buy_btc,
      sum(filt.omega_sell_btc) as total_omega_sell_btc,
      sum(filt.sab_buy_btc) as total_sab_buy_btc
    from
      filt
    group by
      filt.customer_id,
...
select
  ...
  a.total_omega_buy_usdt,
  a.total_omega_sell_usdt,
  a.total_sab_buy_usdt,
  lr.closing_balance_usdt,
  a.total_omega_buy_btc,
  a.total_omega_sell_btc,
  a.total_sab_buy_btc,
  lr.closing_balance_btc,
  lr.closing_balance_zar,
  lr.total_invested_usdt,
  lr.total_deployed_usdt,
  lr.total_fees_paid_usdt,
  lr.total_fees_paid_btc,
  lr.total_fees_paid_zar,
