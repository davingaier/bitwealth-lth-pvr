// Fetch yesterday's CI bands and upsert into lth_pvr.ci_bands_daily
...
Deno.serve(async (req: Request) => {
  ...
  async function fetchJSONWithRetry(url: string, headers: Record<string,string>, attempts = 5) {
    ...
    throw new Error(`CI fetch failed after ${attempts} attempts: ${url}`);
  }
  // --------------------------------------------------------------
  ...
  const org_id =
    ...
    Deno.env.get("ORG_ID") || null;

  const mode: string = (typeof body.mode === "string" && body.mode) || "static";
  ...
  const days  = Number.isFinite(Number(body?.days)) && Number(body.days) > 0 ? Number(body.days) : 5;
  if (!org_id) {
  ...
  if (!apiKey) return new Response("CI_API_KEY missing", { status: 500 });
  // Build URL; prefer explicit range else a short trailing window (days) for self-heal
  let url = `https://chartinspect.com/api/v1/onchain/lth-pvr-bands?mode=${encodeURIComponent(mode)}`;
  if (start) url += `&start=${encodeURIComponent(start)}`;
  if (end)   url += `&end=${encodeURIComponent(end)}`;
  if (!start && !end && days > 1) url += `&days=${days}`;

  console.info("CI GET", url);
  const json = await fetchJSONWithRetry(url, { "X-API-Key": apiKey });
  const data = Array.isArray(json?.data) ? json.data : [];
  console.info("ci status ok; rows:", data.length);

  console.info("ci fetch ok; rows:", data.length, "first:", data[0]?.date, "last:", data.at(-1)?.date);
  if (!Array.isArray(data) || data.length === 0) {
    return new Response("No data", { status: 502 });
  }

  // helpers (asNum, pickNum, pickByRegex) can remain where they are above
  ...
  const toRec = (r: any) => {
    ...
    const val = (...aliasesOrRegex: any[]) =>
      pickNum(r, ...aliasesOrRegex.filter(a => typeof a === "string")) ??
    ...
    return {
      ...
      price_at_mean:    val("price_at_pvr_mean", "lth_realized_price", "mean_price"),
  ...
  const up = await sb.from("ci_bands_daily").upsert(records, {
  ...
  try {
    ...
    if (!chk.data) {
      ...
      const url1d = `https://chartinspect.com/api/v1/onchain/lth-pvr-bands?mode=${encodeURIComponent(mode)}&days=1`;
      const json2 = await fetchJSONWithRetry(url1d, { "X-API-Key": apiKey }, 3);
      const data2 = Array.isArray(json2?.data) ? json2.data : [];
      if (data2.length) {
        const rec2 = data2.map(toRec);
        await sb.from("ci_bands_daily").upsert(rec2, { onConflict: "org_id,date,mode" });
      }
    }
  } catch (e) {
