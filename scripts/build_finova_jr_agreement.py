"""
BitWealth Asset Managers (Pty) Ltd  ×  Finova Capital (Pty) Ltd
Juristic Representative (Mandate) Agreement — s13(1)(b) FAIS Act
Output: docs/FSCA Compliance/BitWealth_Finova_JR_Agreement_v1.docx
...
Key facts captured:
  ...
  · FSP = Finova Capital (Pty) Ltd (FSP No. 21095)
  · Product = Crypto Assets — INTERMEDIARY services only (no advice)
...
r = pa.add_run("FINOVA CAPITAL (PTY) LTD")
r.bold = True; r.font.size = Pt(12); r.font.name = BODY_FONT; r.font.color.rgb = DARK
plain(doc, "Registration number: [[Finova registration number — confirm]]",
...
clause(doc, "1.2", "The financial services under this Agreement are limited to "
        "INTERMEDIARY SERVICES (as defined in the FAIS Act) in respect of Crypto Assets. The "
        "Juristic Representative is NOT authorised to furnish advice unless and until this "
        "Agreement and Schedule A are amended in writing to include advice and the applicable "
        "fit-and-proper competency requirements are met.")
...
jr = [
    ...
    ("2.2", "The Juristic Representative shall at all times comply with the lawful "
            "instructions of the appointed compliance officer of the Appointing FSP, being "
            "[[Finova compliance officer — name & contact to confirm]]."),
    ...
    ("2.8", "The Juristic Representative shall appoint at least one Key Individual who will "
            ...
            "appointed Key Individual is [[Key Individual — name & ID to confirm]]."),
    ...
    ("2.10", "The Key Individual must maintain, at all times, a working knowledge and "
             ...
             "Representative rendering intermediary services in respect of Crypto Assets, and "
             "comply therewith."),
]
for n, txt in jr:
    clause(doc, n, txt)
...
clause(doc, "2.19", "PROFESSIONAL INDEMNITY. The Appointing FSP may, in its discretion, "
        "require the Juristic Representative to maintain in force suitable professional "
        "indemnity cover and/or fidelity insurance cover of a minimum of "
        "[[PI cover amount — to confirm with Finova (template default R1 million each)]], "
        "which cover shall include the matters referred to below.", bold_lead=None)
...
clause(doc, "4.1", "NO MONTHLY FEE. The Parties agree that the Juristic Representative shall "
        "not be required to pay the Appointing FSP any fixed monthly fee for the use of the "
        "Appointing FSP\u2019s operational oversight, licence and processes.", bold_lead=None)

clause(doc, "4.2", "REVENUE SHARE. The Juristic Representative renders intermediary services "
...
hdr = ["Fee type", "Juristic Representative (BitWealth)", "Appointing FSP (Finova)"]
for i, h in enumerate(hdr):
    c = tbl.cell(0, i)
    c.text = ""; run = c.paragraphs[0].add_run(h)
...
fee_rows = [
    ...
    ["Exchange-fee share (VALR) \u2014 not shared", "100%", "0%"],
]
for r, row in enumerate(fee_rows, 1):
    bg = LGREY_HEX if r % 2 == 0 else "FFFFFF"
    for cidx, val in enumerate(row):
        c = tbl.cell(r, cidx); c.text = ""
...
clause(doc, "4.3", "REVERSE REFERRAL. In addition, the Appointing FSP shall pay the Juristic "
        ...
        "Representative to the Appointing FSP.", bold_lead=None)
clause(doc, "4.4", "DEFINITIONS. \u201cPerformance fee\u201d, \u201cplatform fee\u201d and "
        "\u201cmanagement fee\u201d bear the meanings given to them in the client mandate/agreement "
        "concluded with each client. \u201cNet revenue\u201d and \u201cnet fee\u201d mean the amount "
        "actually received by the Appointing FSP after deduction of any product-provider or "
        "third-party charges and any statutory levies, but before income tax.")
clause(doc, "4.5", "PAYMENT. The Appointing FSP shall pay the Juristic Representative its "
        "share of fees and revenue on or before the last business day of the month following the "
        "month in which the relevant fee or revenue is actually received by the Appointing FSP.")
clause(doc, "4.6", "PAYMENT DETAILS. Payments shall be made by electronic funds transfer to "
        "the Juristic Representative\u2019s nominated bank account: [[BitWealth banking details \u2014 "
        "to insert]].")
clause(doc, "4.7", "All fees charged to clients shall be disclosed to clients in accordance "
...
for n, txt in [
    ("5.1.1", "The arbitration shall be held informally at [[arbitration city \u2014 e.g. "
              "Johannesburg]], and the arbitrator shall be agreed between the Parties within 5 "
              "(five) business days, failing which the arbitrator shall be nominated by the "
              "Chairperson of the [[relevant]] Bar Council or the President of the [[relevant]] "
              "Law Society (or its successor). The arbitrator shall be an attorney or advocate "
              "of at least 10 (ten) years\u2019 standing with experience and knowledge of financial "
              "services law and the FAIS Act, and with no interest in the proceedings."),
...
plain(doc, "The Appointing FSP: [[Finova registered address, email address to confirm]]",
      space_after=2)
plain(doc, "The Juristic Representative: [[BitWealth registered address, email address to insert]]",
      space_after=6)
...
for item in [
    ...
    "BitWealth\u2019s appointed Key Individual \u2014 name and ID, and confirmation of fit-and-proper "
    "status (clauses 2.8\u20132.9).",
    "Whether Finova requires professional indemnity / fidelity cover, and the minimum amount "
    "(clause 2.19; template default R1 million each).",
    "Confirmation of third-party providers to be named in clause 2.12 (e.g. VALR).",
    "BitWealth banking details for fee payment (clause 4.6).",
    "Arbitration city and the relevant Bar Council / Law Society region (clause 5.1.1).",
    "Finova and BitWealth registered addresses and email addresses for notices (clause 8.6).",
    "Effective / commencement date of the Agreement.",
    "Confirm the fee split, reverse-referral rate, and that exchange fees are not shared "
    "(clauses 4.2\u20134.3).",
]:
...
for line in ["Signed: __________________________     Name: [[Finova signatory]]",
...
out = OUT_DIR / "BitWealth_Finova_JR_Agreement_v1.docx"
doc.save(out)
print(f"Saved: {out}")
