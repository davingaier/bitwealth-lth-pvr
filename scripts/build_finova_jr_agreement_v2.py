...
clause(doc, "2.19", "PROFESSIONAL INDEMNITY AND FIDELITY COVER. The Appointing FSP shall "
        ...
        "financial services rendered under this Agreement (as set out in Schedule B). The "
...
clause(doc, "3.3", "The Appointing FSP shall collect all client fees generated by the "
        ...
        "accordance with Schedule B (Fee Schedule).")
...
clause(doc, "4.1", "The fees, revenue share and payment terms applicable to this Agreement are "
        "set out in Schedule B (Fee Schedule), which forms part of this Agreement.")
clause(doc, "4.2", "The Parties agree that Schedule B may be amended, replaced or updated from "
        ...
        "Agreement. A duly signed replacement Schedule B shall supersede the previous Schedule B "
...
clause(doc, "6.5", "CLIENT OWNERSHIP ON TERMINATION (LTH PVR PRODUCT). Notwithstanding any "
        ...
        "continue to be compensated with its agreed fee share (as set out in Schedule B) "
...
doc.add_page_break()
section(doc, None, "SCHEDULE A \u2014 FINANCIAL PRODUCTS AND SERVICES")
plain(doc, "The Juristic Representative is authorised to render the financial services marked "
           "below. Products marked '\u2713 Authorised' are currently authorised. Products marked '\u2014' "
           "are listed to accommodate future authorisation by written Schedule A amendment.",
      space_after=4)
plain(doc, "Currently authorised: Crypto Assets \u2014 Category II (Discretionary FSP Services) only. "
           "All other products require a written Schedule A amendment, FSCA notification, and "
           "confirmation that fit-and-proper requirements are met before activation.",
...
ALL_PRODS = [
    ...
    ("1.7",  "Pension Fund Benefits (excl. retail pension benefits)"),
    ("1.8",  "Shares"),
    ("1.9",  "Money Market Instruments"),
    ("1.10", "Debentures and Securitised Debt"),
    ("1.11", "Warrants, Certificates and other instruments acknowledging debt"),
    ("1.12", "Bonds"),
    ("1.13", "Derivative instruments (excluding warrants)"),
    ...
    ("1.17", "Deposits (Banks Act) \u2014 exceeding 12 months"),
    ("1.18", "Deposits (Banks Act) \u2014 12 months or less"),
    ...
    ("1.27", "Crypto Assets (GN 1350, 19 October 2022)"),
]

sa = doc.add_table(rows=len(ALL_PRODS) + 1, cols=6)
sa.style = "Table Grid"; sa.alignment = WD_TABLE_ALIGNMENT.LEFT
for row in sa.rows:
    row.cells[0].width = Cm(1.3); row.cells[1].width = Cm(6.8)
    row.cells[2].width = Cm(1.8); row.cells[3].width = Cm(1.9)
    row.cells[4].width = Cm(2.0); row.cells[5].width = Cm(1.8)
sa_hdr = ["Nr", "Financial Product", "Cat I\nAdvice", "Cat I\nInterm.", "Cat II\nDiscret.", "Status"]
for i, h in enumerate(sa_hdr):
    c = sa.cell(0, i); c.text = ""
    run = c.paragraphs[0].add_run(h)
    run.bold = True; run.font.size = Pt(8); run.font.name = BODY_FONT; run.font.color.rgb = WHITE
    c.paragraphs[0].alignment = WD_ALIGN_PARAGRAPH.CENTER
    set_cell_bg(c, NAVY_HEX)
GREEN_RGB = RGBColor(0x1B, 0x5E, 0x20)
for r, (nr, prod) in enumerate(ALL_PRODS, 1):
    is_crypto = "Crypto" in prod
    bg = "FFFDE7" if is_crypto else (LGREY_HEX if r % 2 == 0 else "FFFFFF")
    vals = [nr, prod, "\u2014", "\u2014",
            "\u2713 Authorised" if is_crypto else "\u2014",
            "Current" if is_crypto else "Future"]
    for ci, val in enumerate(vals):
        c = sa.cell(r, ci); c.text = ""
        run = c.paragraphs[0].add_run(str(val))
        run.font.size = Pt(8.5 if ci == 1 else 9)
        run.font.name = BODY_FONT
        run.bold = (is_crypto and ci == 4)
        run.font.color.rgb = GREEN_RGB if (is_crypto and ci in (4, 5)) else DARK
        c.paragraphs[0].alignment = (
            WD_ALIGN_PARAGRAPH.LEFT if ci == 1 else WD_ALIGN_PARAGRAPH.CENTER)
        set_cell_bg(c, bg)
plain(doc, "To authorise any product marked 'Future', the Parties must execute a replacement "
           "Schedule A, notify the FSCA, and confirm all applicable fit-and-proper competency "
           "requirements are met. No re-execution of the main Agreement is required.",
      italic=True, size=9, color=GREY, space_after=6)
...
# ── Signatures ───────────────────────────────────────────────────────────────
doc.add_page_break()
section(doc, None, "SCHEDULE B \u2014 FEE SCHEDULE")
plain(doc, "This Schedule B forms part of the Juristic Representative Agreement between the "
...
clause(doc, "B.1", "NO MONTHLY FEE. The Juristic Representative shall not be required to pay "
        "the Appointing FSP any fixed monthly fee for the use of the Appointing FSP\u2019s "
        "operational oversight, licence and processes.")
clause(doc, "B.2", "REVENUE SHARE. The Juristic Representative renders intermediary services "
...
clause(doc, "B.3", "REVERSE REFERRAL. In addition, the Appointing FSP shall pay the Juristic "
...
clause(doc, "B.4", "DEFINITIONS. \u201cPerformance fee\u201d, \u201cplatform fee\u201d and "
...
clause(doc, "B.5", "PAYMENT. The Juristic Representative shall invoice the Appointing FSP "
...
clause(doc, "B.6", "PAYMENT DETAILS. Payments shall be made by electronic funds transfer to "
...
plain(doc, "Agreed and accepted \u2014 Schedule B (Fee Schedule):", bold=True, space_after=10)
...
for line in ["Signed: __________________________     Name: Davin Harald Gaier",
             "Capacity: Managing Director              Date: __________________",
             "At: ______________________________",
             "Witness 1: _______________________     Witness 2: _______________________"]:
    plain(doc, line, space_after=6)
